...
def metricCalculation(inputfile,outfile):
	...
	modelfile = inputData['models']
	importancefile = inputData['featureimportance']
	...
	if 'feature_selection_algorithm' in inputData:
		...
		for r in db.FeatureSelection.find(inputData['feature_selection_parameters'],{"_id":1}):
			fs_id = r['_id']
	assert len(resultsfile) == len(modelfile), "results not equal model"
	assert len(resultsfile) == len(importancefile), "results not equal importance"
	for r,m,f,i in zip(resultsfile,modelfile,importancefile,range(len(resultsfile))):
		test_labels = pd.read_csv(r)
		true_labels = test_labels[label]
		predictions = test_labels['predictions']
		true_labels_enc = pd.get_dummies(true_labels)
		predictions_enc = pd.get_dummies(predictions)
		...
		try:
			...
			# mcc might work in a later version of sklearn that added multiclass
			mcc = 0.0
		try:
			...
			std = 0.0
		try:
			roc_auc = float(roc_auc_score(true_labels_enc,predictions_enc,average="macro"))
		except ValueError:
			roc_auc = 0.0
		objId = int(db.seqs.find_and_modify(query={ 'collection' : 'result' },update={'$inc': {'id': 1}},fields={'id': 1, '_id': 0},new=True).get('id'))
		db.Result.insert({"_id":objId,"pp_id":pp_id,"sd_id":sd_id,"file_id":i+1,"feature_extraction_id":fe_id,"feature_selection_id":fs_id,"algo_id":algo_id,"accuracy":accuracy,"precision":precision,"recall":recall,"f1_score":f1_Score,"roc_auc":roc_auc,"cohen_kappa":cohen_kappa,"standard_deviation":std,"model_path":m,"feature_importance_path":f})
		metric_calc = dict()
		...
		metric_calc['cohen_kappa']=cohen_kappa
